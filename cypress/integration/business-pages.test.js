...
describe('Business pages test ', () => {
  ...
  //   cy.get('[data-cy=Код]').contains(config.tableHeadRowTitles.businessPages[1]);
  ...
  it('pass validation', () => {
    ...
    cy.get('.MuiFormHelperText-root.MuiFormHelperText-contained.Mui-error').last().should('be.visible'); 
    cy.get('.MuiFormHelperText-root.MuiFormHelperText-contained.Mui-error').last().contains('Введіть заголовок'); 
    cy.get('.MuiFormHelperText-root.MuiFormHelperText-contained.Mui-error').first().should('be.visible');  
    cy.get('.MuiFormHelperText-root.MuiFormHelperText-contained.Mui-error').first().contains('Введіть унікальний ідентифікатор для сторінки'); 
  });
});
