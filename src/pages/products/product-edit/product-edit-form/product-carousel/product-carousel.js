...
import {
    ...
    setFilesToDelete,
    setFilesToUpload, setPrimaryImageToUpload
...
const {imagePrefix, product: {responsive}} = config;
const {
    DELETE_PRODUCT_BTN,
...
const ProductCarousel = ({toggleFieldsChanged}) => {
    ...
    const handleMultipleFilesLoad = async (e) => {
        ...
        if (files && files[0]) {
            toggleFieldsChanged(true)
            const imagesNames = productImages.map(({name}) => name);
            const newImages = Array.from(files).filter(({name}) => !imagesNames.includes(name));
            const results = await Promise.all(convertToBase64(newImages))
            if(results.length) {
                setProductImages((oldImages) => [
                    ...oldImages,
                    ...results.map((image, idx) => ({url: image, prefix: false, name: newImages[idx].name}))
                ])
    ...
    const handleImagesDeleting = (id, url, prefix, name) => {
        let removeProduct;
        if (prefix) {
            const imagesToDelete = product.images.additional.find(({large}) => large === url)
            removeProduct = () => {
        ...
        {productImages.map(({prefix, url, name}, idx) => (
            <div key={url}>
                ...
                <Grid container justify='center' spacing={2}>
                    <Grid item style={{ width: (idx !== 0 && product.images.additional.length > 1) || !prefix  ? '' : '100%' }}>
                        <Box mt={1}>
                            <UploadButtonContainer
                                className={styles.imageBtn}
                                buttonLabel={idx === 0 ? 'ОНОВИТИ ГОЛОВНЕ ФОТО' : 'НОВІ ФОТО'}
                                onChangeHandler={idx === 0 ? handlePrimaryImageLoad : handleMultipleFilesLoad}
                                startIcon={true}
                                multiple={idx !== 0}
                            />
                        </Box>
                    </Grid>
                    {(idx !== 0 && !prefix) || product.images.additional.length > 1  ?
                        <Grid item>
                            <Box mt={1}>
                                <Button
                                    className={styles.imageBtn}
