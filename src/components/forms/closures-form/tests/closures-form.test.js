...
import { mockClosure, files, target } from './mockClosures';
...
describe('closure form tests', () => {
  ...
  beforeEach(() => {
    ...
    spyOnUseDispatch.mockImplementation(() => mockDispatch);
  ...
  it('Should render 2 buttons and 3 inputs', () => {
    expect(component.find('input')).toHaveLength(3);
    expect(component.find('button')).toHaveLength(2);
  });

  it(`Should render go-closure button with '${GO_BACK_TITLE}' label`, () => {
  ...
  it('should render ImageUploadPreviewContainer component', () => {
    ...
    expect(wrapper.exists(ImageUploadContainer)).toBe(true);
  });
  ...
  it('Should test closureImage', () => {
    const reader = FileReader.mock.instances[0];
    reader.onload(target);
    expect(mockSetClosureImage).toHaveBeenCalled();
    expect(mockSetClosureImage).toHaveBeenCalledWith('foo');
  });
  ...
  it('Should render closure-form', () => {
    expect(component).toBeDefined();
    expect(component).toHaveLength(1);
  });

  it('Should have appropriate prop types', () => {
    expect(ClosureForm.propTypes.id).toBe(PropTypes.string);
    expect(ClosureForm.propTypes.edit).toBe(PropTypes.bool);
  });
