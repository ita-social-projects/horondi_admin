...
export function* handleRecentCommentsLoad({ payload }) {
  try {
    ...
    if (comments?.message === AUTH_ERRORS.ACCESS_TOKEN_IS_NOT_VALID) {
      yield call(handleRefreshTokenPair);
      yield handleRecentCommentsLoad({ payload });
    } else {
...
export function* handleCommentDelete({ payload }) {
  try {
    ...
    if (comment?.message === AUTH_ERRORS.ACCESS_TOKEN_IS_NOT_VALID) {
      yield call(handleRefreshTokenPair);
      yield handleCommentDelete({ payload });
    } else {
...
export function* handleCommentUpdate({ payload }) {
  ...
  try {
    ...
    if (commentData?.message === AUTH_ERRORS.ACCESS_TOKEN_IS_NOT_VALID) {
      yield call(handleRefreshTokenPair);
      yield handleCommentUpdate({ payload });
    } else {
