...
const ConstructorPageContainer = ({
  ...
  const { CREATE_ITEM_TITLE } =
    config.buttonTitles.constructorPageTitles[itemKey];
  const { mainPageTitle } = config.titles.constructorPageTitles[itemKey];
  ...
  const constructorItems = items.map((item) => {
    ...
    const editHandler = () => dispatch(push(`/${itemKey}s/${item._id}`));
